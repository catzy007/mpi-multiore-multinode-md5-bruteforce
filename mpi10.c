...
//some variables
	...
	int s1s2,s1,s2,s3,s4,s5,s6,s7,s8,step=8; //loop variables for key generator
	...
	char data[]="afa35a2374eaa962fc6089f231d94ac1"; //testcase
...
//mpi algorithm begin
	...
	for(i=0;i<processor;i++){
		if(rank==i){
			...
			for(s4=0;s4<62;s4++){
			for(s3=0;s3<62;s3++){
			for(s1s2=i;s1s2<3844;s1s2+=processor){ //3844=62^2
			s2=s1s2/62;
			s1=s1s2%62;
				temp[0]=dict[s8];
				temp[1]=dict[s7];
				...
				//printf("%d %s %s %d=%c %d=%c\n",s1s2,temp,hash,s2,temp[s2],s1,temp[s1]); //debug_line_can_be_removed
